...
import { buyerInputSchema, BuyerInput, cityEnum, propertyTypeEnum, bhkEnum, purposeEnum, timelineEnum, sourceEnum } from "@/lib/validators/buyer";
...
export default function BuyerDetailPage() {
  const { id } = useParams();
  const [buyer, setBuyer] = useState<BuyerInput & { updatedAt: string; tags?: string[] }>({} as any);
  const [history, setHistory] = useState<BuyerHistoryEntry[]>([]);
  const [submitting, setSubmitting] = useState(false);
  const [tagsInput, setTagsInput] = useState<string>("");

  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<BuyerInput>({
  ...
  const onSubmit = async (formData: BuyerInput) => {
    setSubmitting(true);
    try {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <Select {...register("city")}>
            ...
            <SelectContent>
              {cityEnum.options.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div>
          ...
          <Select {...register("propertyType")}>
            ...
            <SelectContent>
              {propertyTypeEnum.options.map(pt => <SelectItem key={pt} value={pt}>{pt}</SelectItem>)}
            </SelectContent>
          </Select>
        ...
        {["Apartment", "Villa"].includes(propertyType || "") && (
          <div>
            ...
            <Select {...register("bhk")}>
              ...
              <SelectContent>
                {bhkEnum.options.map(b => <SelectItem key={b} value={b}>{b}</SelectItem>)}
              </SelectContent>
            </Select>
        ...
        <div>
          ...
          <Select {...register("purpose")}>
            ...
            <SelectContent>
              {purposeEnum.options.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div>
          ...
          <Select {...register("timeline")}>
            ...
            <SelectContent>
              {timelineEnum.options.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div>
          ...
          <Select {...register("source")}>
            ...
            <SelectContent>
              {sourceEnum.options.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
            </SelectContent>
          </Select>
          ...
          <ul className="space-y-2">
            {history.map(h => (
              <li key={h.id} className="p-2 border rounded dark:border-gray-600">
                <div><strong>{new Date(h.changedAt).toLocaleString()}</strong> by {h.changedBy}</div>
